...
struct inpub_binder<N, Head, Tail...> {
    static void bind(
...
struct inpub_binder<N, char*, Tail...> {
    static void bind(
        ...
        inpub_binder<N + 1, Tail...>::bind(bindParameters, tail...);
    }
};
template <size_t N, typename... Tail>
struct inpub_binder<N, const char*, Tail...> {
    static void bind(
        std::vector<MYSQL_BIND>* const bindParameters,
        const char* const& value,
        const Tail& ... tail
    ) {
        inpub_binder<N, char*, Tail...>::bind(
            bindParameters,
...
// ******************************************
template <size_t N, typename... Tail>
struct inpub_binder<N, std::string, Tail...> {
    static void bind(
        std::vector<MYSQL_BIND>* const bindParameters,
...
template <size_t N, typename... Tail> \
struct inpub_binder<N, type, Tail...> { \
    static void bind( \
        std::vector<MYSQL_BIND>* const bindParameters, \
        const type& value, \
        const Tail&... tail \
...
void bindInputs(
    ...
    InputBinderPrivate::inpub_binder<0, Args...>::bind(
        inputBindParameters,
        args...);
}
